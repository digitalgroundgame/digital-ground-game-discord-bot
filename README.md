...
Create `.env`, fill in the required Discord values, then start the bot and its
...
The API health endpoint is available at http://localhost:3001/health, and the
database is persisted in `data/`. Stop the environment with:
...
### Optional: CRM-backed attendance

Local development can run without CRM attendance integration. If
`CRM_API_URL` and `CRM_API_TOKEN` are unset while `NODE_ENV` is not `production`,
the attendance commands and voice-state attendance handler are disabled. Set
both values to enable them. Production startup still requires both values.
...
| `/roles`                                       | Slash     | Admin-only, private role inventory for the current server. Lists non-managed role names and IDs for configuring this bot in a new server.                                                                                                                                                                         | [`src/commands/chat/roles-command.ts`](src/commands/chat/roles-command.ts)                                                                                                                                 |
| Role gates                                     | Config    | The tracker MVP uses role names, not server-specific IDs. The current mock policy maps `Above All`, `King Froggo`, and `Below All` to create/edit permissions; copy the example policy when configuring another server.                                                                                           | [`config/tracker-permissions.json`](config/tracker-permissions.json), [`config/tracker-permissions.example.json`](config/tracker-permissions.example.json)                                                 |
| `/tracker-panel`                               | Slash     | Posts an interactive PNG-backed project tracker panel. Users can refresh progress, open the full project → milestone → task tree, filter the tree by project/status, choose tasks by `#ID · name`, update status, assign members, create projects and milestones, add tasks, and complete tasks with buttons and Discord dropdowns. | [`src/commands/chat/project-tracker-panel-command.ts`](src/commands/chat/project-tracker-panel-command.ts), [`src/buttons/project-tracker-panel-buttons.ts`](src/buttons/project-tracker-panel-buttons.ts) |
| `/project`, `/milestone`, `/task`, `/progress` | Slash     | Project tracker MVP command fallback for creating and viewing projects, milestones, tasks, assignments, statuses, and progress.                                                                                                                                                                                   | [`src/commands/chat/project-tracker-commands.ts`](src/commands/chat/project-tracker-commands.ts)                                                                                                           |
...
**Not used yet:** message (context-menu) commands are not defined ([`MessageCommandMetadata`](src/commands/metadata.ts) is empty). Reaction handlers are wired up in [`src/start-bot.ts`](src/start-bot.ts) but none are configured. The tracker panel is the first active custom-button workflow.
