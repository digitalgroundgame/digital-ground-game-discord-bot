...
Generate a shared secret for the authenticated `/guilds` and `/presence` API endpoints:
...
| --------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dev`                | Slash     | Required option `command` (currently `info`): shows bot developer diagnostics (Node, TypeScript, discord.js version, server count, memory, hostname, and IDs). Restricted to users in `DISCORD_BOT_DEVELOPER_IDS`. Discord also marks the command as Administrator-only; the bot double-checks developer IDs. | [`src/commands/chat/dev-command.ts`](src/commands/chat/dev-command.ts)                                                                                                       |
...
The Discord client, HTTP API, integrations, and scheduled jobs all run in the single process composed by [`src/start-bot.ts`](src/start-bot.ts).
